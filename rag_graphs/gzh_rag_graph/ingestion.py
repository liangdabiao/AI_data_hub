...
gzh_articles_retriever = Chroma(
    collection_name=os.getenv('VECTOR_DB_COLLECTION'),
    persist_directory=os.getenv("VECTOR_DB_DIRECTORY"),
    embedding_function=OpenAIEmbeddings(base_url=os.getenv("OPENAI_BASE_URL"))
).as_retriever()

class DocumentSyncManager:
    def __init__(self):
        self.mongo_client = MongoDBClient()
        self.gzh_collection = self.mongo_client.get_collection(collection_name=os.getenv('GZH_COLLECTION_NAME'))
    ...
    def fetch_unsynced_documents(self):
        ...
        """
        return self.gzh_collection.find({'synced': False}, {'_id': 1, 'description': 1}).limit(10)

    def mark_documents_as_synced(self, document_ids: List):
        """
        Marks the provided document IDs as synced in the database.
        """
        result = self.gzh_collection.update_many(
            {'_id': {'$in': document_ids}},
            {'$set': {'synced': True}}
        )
    ...
    def store_documents_in_chroma(self, doc_splits: List[Document]):
        ...
        vectorstore = Chroma.from_documents(
            ...
            embedding=OpenAIEmbeddings(base_url=os.getenv("OPENAI_BASE_URL")),
